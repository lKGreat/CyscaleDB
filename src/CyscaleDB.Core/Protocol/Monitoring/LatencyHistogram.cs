...
public sealed class LatencyHistogram
{
    ...
    /// <summary>
    /// Gets the bucket distribution.
    /// </summary>
    public LatencyBucket[] GetBuckets()
    {
        var result = new LatencyBucket[_buckets.Length];
        ...
        lock (_lock)
        {
            ...
            for (int i = 0; i < _buckets.Length; i++)
            {
                result[i] = new LatencyBucket
                {
                    LowerBound = prevBoundary,
                    UpperBound = BucketBoundaries[i],
                    Count = _buckets[i],
                    Percentage = _count > 0 ? (double)_buckets[i] / _count * 100 : 0
                };
                prevBoundary = BucketBoundaries[i];
            }
        }
...
public sealed class LatencyBucket
{
    ...
    public string Label => UpperBound < double.MaxValue
        ? $"{LowerBound:F0}-{UpperBound:F0}ms"
        : $"{LowerBound:F0}ms+";

    public override string ToString()
    {
        return $"{Label}: {Count} ({Percentage:F1}%)";
    }
}
