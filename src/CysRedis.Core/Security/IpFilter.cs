...
public sealed class IpFilter
{
    ...
    /// <summary>
    /// Gets the number of denied connections.
    /// </summary>
    public long DeniedCount => Interlocked.Read(ref _deniedCount);
    ...
    public IpFilter(IpFilterOptions options)
    {
        ...
        if (_options.Enabled)
        {
            Logger.Info("IP filter initialized: mode={0}, whitelist={1}, blacklist={2}",
    ...
    public bool IsAllowed(IPAddress address)
    {
        ...
        else
        {
            ...
            Logger.Warning("Connection denied for IP: {0}", address);
        }
    ...
    /// <summary>
    /// Checks if a string IP address is allowed.
    /// </summary>
    ...
    public void AddToWhitelist(string ipOrCidr)
    {
        if (TryParseNetwork(ipOrCidr, out var network))
        {
            ...
            Logger.Debug("Added network to whitelist: {0}", ipOrCidr);
        }
        else if (IPAddress.TryParse(ipOrCidr, out var ip))
        {
            _whitelistIps[ip] = true;
            Logger.Debug("Added IP to whitelist: {0}", ipOrCidr);
        }
        else
        {
            Logger.Warning("Invalid IP/CIDR format: {0}", ipOrCidr);
        }
    }
    ...
    public void AddToBlacklist(string ipOrCidr)
    {
        if (TryParseNetwork(ipOrCidr, out var network))
        {
            ...
            Logger.Debug("Added network to blacklist: {0}", ipOrCidr);
        }
        else if (IPAddress.TryParse(ipOrCidr, out var ip))
        {
            _blacklistIps[ip] = true;
            Logger.Debug("Added IP to blacklist: {0}", ipOrCidr);
        }
        else
        {
            Logger.Warning("Invalid IP/CIDR format: {0}", ipOrCidr);
